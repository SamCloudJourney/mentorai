...
| **BLMN** | ~~4/5~~ → **2.5/5** | ⚠️ **NOT a trough — casual dining is *winning* the cycle** | Reframed by deeper DD: the *segment* is share-**gaining** (Chili's/LongHorn/TXRH +4–7% traffic) while **Outback bleeds −2.4% traffic** → a *single-brand secular/execution* problem, not cyclical (no "rising tide" safety net). Corrections: CEO is ex-Delta/Six Flags (**not** ex-Darden — that's a board member), Starboard **trimmed to 4.9% voting**, stock already **+55% off its March low** (6x→~9–10x). Prove-it turnaround, not a coiled spring. *Great example of the cycle lens catching a misclassification.* |
...
6. **Auto parts** → cyclical + EV-transition; the beaten-down deep-value names are **Adient** (seating) and **`AXL`→DCH** (driveline, ~4x EV/EBITDA, ~20% FCF yield), while **Lear** already re-rated +60%. *(Final synthesis pending.)*
